...
public class Task2 {
    ...
    private String getFuel(char letter){
        //No letter for other types of fuels in a given model
        switch(letter){
            case 'N':
                return "Petrol";
            case 'R':
    ...
    private String getAirCon(char letter){
        switch(letter){
            case 'N':
                return "No AC";
            case 'R':
    ...
    private String buildOutputString(){
        ...
        for (int i = 0; i < vehicles.size(); i++) {
            ...
            line = "{"+vehicles.get(i).get("name").toString()+"}"+" - "+
                    ...
                    "{"+getFuel(sipp.charAt(FUEL_CON_LETTER_INDEX))+"}"+" - "+
                    "{"+getAirCon(sipp.charAt(FUEL_CON_LETTER_INDEX))+"}\n";
            output.append(line);
        }
